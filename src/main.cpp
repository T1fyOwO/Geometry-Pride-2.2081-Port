...
CCSpriteFrame* updateSpriteCache(
    std::string pride = Mod::get()->getSettingValue<std::string>("woke")
) {
    auto cache = CCSpriteFrameCache::get();
    auto wokeFrame = cache->spriteFrameByName(
        fmt::format("{}.png"_spr, pride).c_str()
    );
...
class $modify(MyLoadingLayer, LoadingLayer) {
    static void onModify(auto& self) {
        Result<> res = self.setHookPriorityPre(
            "LoadingLayer::init",
            Priority::Late
        );
        ...
        if (res.isErr()) {
            geode::log::warn(
                "Failed to set hook priority for LoadingLayer::init: {}",
                res.unwrapErr()
            );
        }
    }
    ...
    bool init(bool p0) {
        ...
        CCSpriteFrameCache::get()->addSpriteFramesWithFile(
            "WokeSheet.plist"_spr
        );

        if (!LoadingLayer::init(p0))
            return false;

        getChildByType<CCSprite*>(1)->setDisplayFrame(
            updateSpriteCache()
        );
